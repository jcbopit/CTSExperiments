...
# This file was *autogenerated* from the file DFA.sage
...
def PolyAutoGeneral(P, Q, p, state_bound, fail_on_zero): # Returns state machine for ct[P^nQ] mod p as a list of states labeled by polynomials Q' and correspondingly indexed transitions
    ...
    if(fail_on_zero and R(Q).constant_coefficient() == _sage_const_0 ):
        return None
    
    k = _sage_const_0  # The index of the current state being processed for transitions
    ...
    while (k < len(states)): # Continue while there are new states Q' = states[k]
        for i in range(p): # Add transitions for states[k]
            ...
            if (fail_on_zero and R(new_state).constant_coefficient() == _sage_const_0 ): # If a zero state is found, return None if fail_on_zero is true
                    return None
